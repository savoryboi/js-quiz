    ...
    function displayBoard(){
        storageData.unshift(playerData);
        localStorage.setItem('scores', JSON.stringify(storageData));
        JSON.parse(localStorage.getItem('scores'));
        
        $(storageData).each(function () {
            for(var i = 0; i < storageData.length; i++){
                var newLi = $(leaderboardEl).append(`<li>${storageData[i]}</li>`);
            }
